...
export function ready(this: { $?: Record<string, HTMLElement> }) {
    panelElements = this.$ || {};
...
async function loadConfig() {
    try {
        currentConfig = normalizeConfig(await requestPackage('query-config'));
    } catch {
        currentConfig = makeDefaultConfig();
    }
...
function normalizeConfig(config: any): BindToolConfig {
    ...
    const mergedRules = mergeMissingDefaultRules(rules);
    ...
    return {
        ...
        rules: mergedRules.length > 0 ? mergedRules : defaultRules.map((rule) => ({ ...rule })),
    };
}

function mergeMissingDefaultRules(rules: SimpleRule[]): SimpleRule[] {
    const prefixes = new Set(rules.map((rule) => rule.prefix));
    return [
        ...rules,
        ...defaultRules.filter((rule) => !prefixes.has(rule.prefix)).map((rule) => ({ ...rule })),
    ];
}
...
function renderConfigSafely() {
    try {
        renderConfig();
    } catch (error) {
        window.setTimeout(() => renderConfigSafely(), 100);
    }
}
...
function bindEvents() {
    ...
    document.addEventListener('click', async (event) => {
        ...
        if (target.id === 'save') {
            currentConfig = normalizeConfig(await requestPackage('save-config', collectConfig()));
            renderConfigSafely();
            setStatus('Saved.');
            return;
        }
        ...
        if (target.id === 'reset') {
            currentConfig = normalizeConfig(await requestPackage('reset-config'));
            renderConfigSafely();
    ...
    document.addEventListener('input', onRulesChanged);
    document.addEventListener('change', onRulesChanged);
}
...
function element<T extends HTMLElement = HTMLElement>(key: keyof typeof $): T {
    ...
    if (fromPanel && typeof fromPanel.addEventListener === 'function') {
...
function requestPackage(message: string, ...args: any[]): Promise<any> {
    return new Promise((resolve, reject) => {
        const timer = window.setTimeout(() => reject(new Error(`Request timeout: ${message}`)), 1200);
        const done = (...callbackArgs: any[]) => {
        ...
        try {
            ...
            if (Editor.Message && typeof Editor.Message.request === 'function') {
                Editor.Message.request(PACKAGE_NAME, message, ...args).then((result: any) => {
                    window.clearTimeout(timer);
                    resolve(result);
                }, (error: any) => {
                    window.clearTimeout(timer);
            ...
            reject(new Error('Editor IPC is unavailable.'));
        } catch (error) {
            window.clearTimeout(timer);
            reject(error);
        }
