...
async function querySelectedNodeTree(selectedUuid: string): Promise<any> {
    try {
        return await Editor.Message.request('scene', 'query-node-tree', selectedUuid);
    } catch (error: any) {
        debugBindTool('query selected node tree fallback', error?.message || String(error));
        return querySelectedAssetNodeTree();
    }
}

function mergeMissingDefaultRules(rules: BindRule[]): BindRule[] {
...
function debugBindTool(message: string, data?: any): void {
    if (!DEBUG_BIND_TOOL) {
        return;
    }
...
function isValidSelectedNodeTree(node: any): boolean {
    return Boolean(node && typeof node === 'object' && !Array.isArray(node) && !(node instanceof String));
}

function describeInvalidSelectedTree(node: any): string {
    if (node === null) {
        return 'null';
    }
...
function summarizeNodeTree(node: any): any {
    if (!node) {
        return null;
    }
...
function summarizeAsset(asset: any): any {
...
async function queryOpenedAssetUrl(selectedTree: any): Promise<string | null> {
    ...
    try {
        ...
        return asset?.url || asset?.source || null;
    } catch (error: any) {
        debugBindTool('queryOpenedAssetUrl fallback after error', error?.message || String(error));
        return null;
    }
}
...
async function resolveBindingNodeTree(openedAssetUrl: string, selectedTree: any): Promise<any> {
    ...
    try {
        ...
        debugBindTool('resolve binding node tree failed', error?.message || String(error));
        return selectedTree;
    }
}

async function querySelectedAssetNodeTree(): Promise<any | null> {
...
function buildSerializedNodeTree(data: any[], rootName: string): any | null {
    const rootId = Number.isInteger(data[0]?.data?.__id__)
        ? data[0].data.__id__
        : data.findIndex((item) => item?.__type__ === 'cc.Node' && item?._name === rootName);

    if (!Number.isInteger(rootId) || !data[rootId] || data[rootId].__type__ !== 'cc.Node') {
        return null;
    }

    const cloneNode = (nodeId: number): any | null => {
        ...
        const children = Array.isArray(node._children)
            ? node._children
                .map((child: any) => cloneNode(child?.__id__))
                .filter(Boolean)
            : [];
...
function findAssetUrlByNodeTree(selectedTree: any): string | null {
    ...
    debugBindTool('scan asset candidates', {
        ...
        candidates: candidates.map((file) => relative(Editor.Project.path, file).replace(/\\/g, '/')).slice(0, 20),
    });
    ...
    for (const file of candidates) {
        try {
            ...
            debugBindTool('inspect asset candidate', {
                file: relative(Editor.Project.path, file).replace(/\\/g, '/'),
                nodeCount: nodes.length,
                firstNames: nodes.map((node) => node?._name).filter(Boolean).slice(0, 12),
                hasRoot,
            });
            if (!hasRoot) {
                continue;
...
function isSerializedAssetUrl(url: string | null): boolean {
...
function getNodeName(node: any): string {
    return String(readDumpValue(node?.name) || readDumpValue(node?._name) || 'Node');
}

function getUuid(node: any): string {
    return String(readDumpValue(node?.uuid) || readDumpValue(node?._id) || readDumpValue(node?._uuid) || node?.uuid || '');
}
...
function getSerializedScriptTypeNames(scriptAsset: any | null, className: string): string[] {
    return [...new Set([
        scriptAsset?.uuid ? compressUuid(String(scriptAsset.uuid)) : '',
