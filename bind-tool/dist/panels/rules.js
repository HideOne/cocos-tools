...
function normalizeConfig(config) {
    ...
    return {
        ...
        rules: mergedRules.length > 0 ? mergedRules : defaultRules.map((rule) => (Object.assign({}, rule))),
    };
}
function mergeMissingDefaultRules(rules) {
    ...
    return [
        ...rules,
        ...defaultRules.filter((rule) => !prefixes.has(rule.prefix)).map((rule) => (Object.assign({}, rule))),
    ];
}
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            ...
            if (Editor.Message && typeof Editor.Message.request === 'function') {
                Editor.Message.request(PACKAGE_NAME, message, ...args).then((result) => {
                    window.clearTimeout(timer);
                    resolve(result);
                }, (error) => {
                    window.clearTimeout(timer);
