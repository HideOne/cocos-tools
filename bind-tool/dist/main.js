...
async function bindSelectedNode(mode) {
    ...
    debugBindTool('selected node uuid', selectedUuid);
    if (!selectedUuid) {
        throw new Error('Please select a node first.');
    }
    const selectedTree = await querySelectedNodeTree(selectedUuid);
    debugBindTool('selected node tree summary', summarizeNodeTree(selectedTree));
    if (!selectedTree) {
        throw new Error('Cannot read the selected node. Make sure a scene or prefab is open.');
    }
    if (!isValidSelectedNodeTree(selectedTree)) {
        throw new Error(`Cannot read the selected node tree. Scene IPC returned invalid data: ${describeInvalidSelectedTree(selectedTree)}`);
    }
    ...
    const bindingTree = await resolveBindingNodeTree(openedAssetUrl, selectedTree);
    if (bindingTree !== selectedTree) {
        debugBindTool('binding node tree summary', summarizeNodeTree(bindingTree));
    }
    const scriptUrl = makeScriptUrl(openedAssetUrl, className, config.scriptRoot);
    const scan = scanBindings(bindingTree, config);
    ...
    if (mode !== 'generate') {
        if (isSerializedAssetUrl(openedAssetUrl)) {
            propertiesBound = await bindSerializedAssetReferences(openedAssetUrl, scriptUrl, selectedUuid, className, getNodeName(bindingTree), scan);
            componentAttached = propertiesBound > 0 || scan.length === 0;
...
async function querySelectedNodeTree(selectedUuid) {
    try {
        return await Editor.Message.request('scene', 'query-node-tree', selectedUuid);
    }
    catch (error) {
        debugBindTool('query selected node tree fallback', (error === null || error === void 0 ? void 0 : error.message) || String(error));
        return querySelectedAssetNodeTree();
    }
}
...
function normalizeRules(rules) {
    ...
    return mergeMissingDefaultRules(normalized.length > 0 ? normalized : defaultConfig.rules);
}
function mergeMissingDefaultRules(rules) {
    const prefixes = new Set(rules.map((rule) => rule.prefix));
    return [
        ...rules,
        ...defaultConfig.rules.filter((rule) => !prefixes.has(rule.prefix)),
    ];
}
...
function debugBindTool(message, data) {
    if (!DEBUG_BIND_TOOL) {
        return;
    }
    const text = data === undefined
        ? `[${PACKAGE_NAME}:debug] ${message}`
        : `[${PACKAGE_NAME}:debug] ${message} ${safeJson(data)}`;
...
function isValidSelectedNodeTree(node) {
    return Boolean(node && typeof node === 'object' && !Array.isArray(node) && !(node instanceof String));
}
function describeInvalidSelectedTree(node) {
    if (node === null) {
        return 'null';
    }
    if (node === undefined) {
    ...
    if (typeof node === 'string' || node instanceof String) {
        return JSON.stringify(String(node).slice(0, 160));
    }
    return safeJson(node).slice(0, 160);
}
function summarizeNodeTree(node) {
    if (!node) {
        return null;
    }
    ...
    return {
        ...
        childNames: children.map(getNodeName).slice(0, 30),
    };
}
...
async function queryOpenedAssetUrl(selectedTree) {
    const matchedAssetUrl = await findAssetUrlByNodeTreeWithRetry(selectedTree);
    if (matchedAssetUrl) {
        return matchedAssetUrl;
    }
    try {
        const savedId = await Editor.Message.request('scene', 'save-scene');
        debugBindTool('scene save-scene result', savedId);
        if (!savedId) {
            return null;
        }
        const asset = await Editor.Message.request('asset-db', 'query-asset-info', savedId);
        debugBindTool('saved asset info', summarizeAsset(asset));
        return (asset === null || asset === void 0 ? void 0 : asset.url) || (asset === null || asset === void 0 ? void 0 : asset.source) || null;
    }
    catch (error) {
        debugBindTool('queryOpenedAssetUrl fallback after error', error && error.message || String(error));
        return null;
    }
}
async function findAssetUrlByNodeTreeWithRetry(selectedTree) {
    for (let index = 0; index < 5; index += 1) {
        debugBindTool('findAssetUrlByNodeTree attempt', index + 1);
        const url = findAssetUrlByNodeTree(selectedTree);
        if (url) {
            debugBindTool('findAssetUrlByNodeTree matched', url);
            return url;
        }
        await delay(120);
    }
...
async function resolveBindingNodeTree(openedAssetUrl, selectedTree) {
    if (getChildren(selectedTree).length > 0) {
        return selectedTree;
    }
    try {
        ...
        return buildSerializedNodeTree(data, getNodeName(selectedTree)) || selectedTree;
    }
    catch (error) {
        debugBindTool('resolve binding node tree failed', (error === null || error === void 0 ? void 0 : error.message) || String(error));
        return selectedTree;
    }
}
async function querySelectedAssetNodeTree() {
    const selectedAsset = Editor.Selection.getLastSelected('asset') || Editor.Selection.getSelected('asset')[0];
    if (!selectedAsset) {
        return null;
    }
    const asset = await Editor.Message.request('asset-db', 'query-asset-info', selectedAsset);
    if (!(asset === null || asset === void 0 ? void 0 : asset.file) || !/\.(prefab|scene)$/i.test(asset.file)) {
        return null;
    }
    const data = await readJsonFileWithRetry(asset.file);
    return Array.isArray(data) ? buildSerializedNodeTree(data, '') : null;
}
function buildSerializedNodeTree(data, rootName) {
    var _a, _b;
    const rootId = Number.isInteger((_b = (_a = data[0]) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.__id__)
        ? data[0].data.__id__
        : data.findIndex((item) => (item === null || item === void 0 ? void 0 : item.__type__) === 'cc.Node' && (item === null || item === void 0 ? void 0 : item._name) === rootName);
    if (!Number.isInteger(rootId) || !data[rootId] || data[rootId].__type__ !== 'cc.Node') {
        return null;
    }
    const cloneNode = (nodeId) => {
        ...
        const children = Array.isArray(node._children)
            ? node._children
                .map((child) => cloneNode(child === null || child === void 0 ? void 0 : child.__id__))
                .filter(Boolean)
            : [];
        return Object.assign(Object.assign({}, node), { _children: children });
    };
...
function findAssetUrlByNodeTree(selectedTree) {
    ...
    if (!(0, fs_1.existsSync)(assetsDir)) {
        debugBindTool('assets dir missing', assetsDir);
        return null;
    }
    ...
    debugBindTool('scan asset candidates', {
        rootName,
        childNames: [...childNames].slice(0, 20),
        candidateCount: candidates.length,
        candidates: candidates.map((file) => (0, path_1.relative)(Editor.Project.path, file).replace(/\\/g, '/')).slice(0, 20),
    });
    for (const file of candidates) {
        try {
            ...
            debugBindTool('inspect asset candidate', {
                file: (0, path_1.relative)(Editor.Project.path, file).replace(/\\/g, '/'),
                nodeCount: nodes.length,
                firstNames: nodes.map((node) => node === null || node === void 0 ? void 0 : node._name).filter(Boolean).slice(0, 12),
                hasRoot,
            });
            if (!hasRoot) {
                continue;
...
function isSerializedAssetUrl(url) {
    return Boolean(url && /\.(prefab|scene)$/i.test(url));
}
...
function getNodeName(node) {
    return String(readDumpValue(node === null || node === void 0 ? void 0 : node.name) || readDumpValue(node === null || node === void 0 ? void 0 : node._name) || 'Node');
}
function getUuid(node) {
    return String(readDumpValue(node === null || node === void 0 ? void 0 : node.uuid) || readDumpValue(node === null || node === void 0 ? void 0 : node._id) || readDumpValue(node === null || node === void 0 ? void 0 : node._uuid) || (node === null || node === void 0 ? void 0 : node.uuid) || '');
}
function getChildren(node) {
    if (Array.isArray(node === null || node === void 0 ? void 0 : node.children)) {
        return node.children;
    }
    if (Array.isArray(node === null || node === void 0 ? void 0 : node._children)) {
...
function renderProperties(bindings) {
    ...
    public ${binding.propertyName}: ${toScriptTypeName(binding.propertyType)} = null;`).join('\n\n');
}
function toScriptTypeName(typeName) {
    const value = typeName.trim();
    if (value === 'sp' || value.startsWith('sp.')) {
        return value;
    }
    return value.startsWith('cc.') ? value : `cc.${value}`;
}
function renderButtonEvents(buttons) {
    return buttons.map((button) => `        if (this.${button.propertyName}) {
            this.${button.propertyName}.node.on('click', this.${getClickHandlerName(button.propertyName)}, this);
        }`).join('\n\n');
}
...
function mergeCcImports(existing, generated) {
    ...
    const decoratorPattern = /const\s+\{\s*ccclass\s*,\s*property\s*\}\s*=\s*_decorator;\s*/;
    let updated = existing
        .replace(importPattern, '')
        .replace(decoratorPattern, '')
        .replace(/@ccclass\(['"][^'"]+['"]\)/g, '@ccclass')
        .replace(/extends\s+Component\b/g, 'extends cc.Component')
        .replace(/\bButton\.EventType\.CLICK\b/g, "'click'");
    if (!updated.includes('cc._decorator')) {
        updated = `const { ccclass, property } = cc._decorator;\n\n${updated.trimStart()}`;
    }
    return updated;
}
...
async function readAssetText(url) {
    ...
    if (!(asset === null || asset === void 0 ? void 0 : asset.file) || !(0, fs_1.existsSync)(asset.file)) {
        return null;
    }
...
async function bindSerializedAssetReferences(openedAssetUrl, scriptUrl, selectedUuid, className, rootName, bindings) {
    ...
    else if (scriptTypeNames[0] && (targetComponent.__type__ === className || !scriptTypeNames.includes(targetComponent.__type__))) {
        targetComponent.__type__ = scriptTypeNames[0];
    }
    let boundCount = 0;
    for (const binding of bindings) {
...
function getSerializedScriptTypeNames(scriptAsset, className) {
    return [...new Set([
            (scriptAsset === null || scriptAsset === void 0 ? void 0 : scriptAsset.uuid) ? compressUuid(String(scriptAsset.uuid)) : '',
...
function compressUuid(uuid) {
    ...
    const tail = normalized.slice(5);
    let result = head;
    for (let index = 0; index < tail.length; index += 3) {
        const value = parseInt(tail.slice(index, index + 3), 16);
        result += chars[value >> 6];
